...
// Тестовий endpoint
app.get("/", (req, res) => {
    res.json({ message: "Teachers API is working!" });
});
...
app.get("/api/teachers", async (req, res) => {
    try {
        ...
        res.status(500).json({
            message: "Помилка сервера при отриманні вчителів"
        });
    }
});
...
app.get("/api/teachers/:id", async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            message: "Помилка сервера при отриманні вчителя"
        });
    }
});
...
app.post("/api/teachers", async (req, res) => {
    ...
    try {
        ...
        const newTeacher = {
            ...
            subject,
            photo:
                photo || "https://cdn-icons-png.flaticon.com/512/168/168726.png" // Дефолтне фото
        };
        ...
        res.status(500).json({
            message: "Помилка сервера при створенні вчителя"
        });
    }
});
...
app.put("/api/teachers/:id", async (req, res) => {
    ...
    try {
        ...
        if (found) {
            ...
            res.status(404).json({
                message: "Вчителя не знайдено для оновлення"
            });
        }
    } catch (error) {
        console.error("Помилка при оновленні вчителя:", error);
        res.status(500).json({
            message: "Помилка сервера при оновленні вчителя"
        });
    }
});
...
app.delete("/api/teachers/:id", async (req, res) => {
    ...
    try {
        ...
        if (teachers.length < initialLength) {
            ...
            res.status(404).json({
                message: "Вчителя не знайдено для видалення"
            });
        }
    } catch (error) {
        console.error("Помилка при видаленні вчителя:", error);
        res.status(500).json({
            message: "Помилка сервера при видаленні вчителя"
        });
    }
});
